    ...
    : IRabbitMqPersistentConnection
{
    ...
    public void Dispose()
    {
        ...
        {
            _logger.LogCritical("An error occurred while disposing the connection: {Message}", ex.Message);
        }
    }
                ...
                .WaitAndRetry(_retryCount,
                    ...
                    (ex, time) =>
                    {
                        _logger.LogWarning(ex,
                            "RabbitMQ Client could not connect to {Server} after {Timeout} with user {UserName} {ExceptionMessage}",
                            _connection?.Endpoint.ToString(),
